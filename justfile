...
minsize command *args:
    RUSTFLAGS="-Ctarget-cpu=x86-64-v3 -Ctarget-feature=+fast-gather" cargo {{command}} --target x86_64-unknown-linux-gnu -Z build-std=std,panic_abort -Z build-std-features=panic_immediate_abort --no-default-features --profile minsize {{args}}

# unwinding and backtraces enabled
releasedebug command *args:
    RUSTFLAGS="-Ctarget-cpu=x86-64-v3 -Ctarget-feature=+fast-gather" cargo {{command}} --features backtrace --profile releasedebug {{args}}

devfast command *args:
    RUSTFLAGS="-Zub-checks -Ctarget-cpu=x86-64-v3 -Ctarget-feature=+fast-gather" cargo {{command}} --features backtrace --profile devfast {{args}}

dev command *args:
    RUSTFLAGS="-Zub-checks -Ctarget-cpu=x86-64-v3 -Ctarget-feature=+fast-gather" cargo {{command}} --features backtrace --profile dev {{args}}

# simple panic handling enabled
release-build-all-targets *args:
    RUSTFLAGS="-Ctarget-cpu=x86-64-v3 -Ctarget-feature=+fast-gather" cargo build --target x86_64-unknown-linux-gnu -Z build-std=std,panic_abort -Z build-std-features= --release {{args}}
    ...
    RUSTFLAGS="-Ctarget-cpu=x86-64-v3 -Ctarget-feature=+fast-gather" cargo zigbuild --target x86_64-apple-darwin -Z build-std=std,panic_abort -Z build-std-features= --release {{args}}
    ...
    RUSTFLAGS="-Ctarget-cpu=x86-64-v3 -Ctarget-feature=+fast-gather" cargo xwin build --target x86_64-pc-windows-msvc -Z build-std=std,panic_abort -Z build-std-features= --release {{args}}
